...
export function getUserData(id: string) {
  if (fs.existsSync(`./database/users/${id}.json`)) {
    return JSON.parse(
      fs.readFileSync(`./database/users/${id}.json`).toString()
    );
  } else {
...
export function writeUserData(id: string, userData: object) {
...
export function getGuildData(id: string) {
  if (fs.existsSync(`./database/guilds/${id}.json`)) {
    return JSON.parse(
      fs.readFileSync(`./database/guilds/${id}.json`).toString()
    );
  } else {
...
export function writeGuildData(id: string, guildData: object) {
  fs.writeFile(
    `./database/guilds/${id}.json`,
